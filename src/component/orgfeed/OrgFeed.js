...
const OrgFeed = () => {
  ...
  const showSetupProfile = () => {                  // USER AUTHENTICATION 
    axios.get(`${BASE_URL}/org/verify`, { withCredentials: true })
      .then((res) => {
      }).catch((err) => {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL}/org/feed/docandrating`, { withCredentials: true }).then((result) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${BASE_URL}/org/feed/liveprojects`, { withCredentials: true })
      .then((res) => {
        const tempArray = [];
  ...
  const Swiped = (direction, index, _id, k) => {
    ...
    if (direction === "right") {
      axios.post(`${BASE_URL}/org/feed/liveprojects/rightswipe`, {
    ...
    if (direction === "left") {
      axios.post(`${BASE_URL}/org/feed/liveprojects/leftswipe`, {
        project_id: k.project._id,
        user_id: k.user.userID._id
      }, { withCredentials: true }).then((res) => {
        cogoToast.success(res.data.message);
